...
async def test_controller_get_should_return_sucess(client, products_url, product_inserted):
    ...
    assert content == {'id': str(product_inserted.id), 'name': 'Iphone 14 Pro Max', 'quantity': 10, 'price': '8.500', 'status': True} # type: ignore
...
async def test_controller_get_should_return_not_found(client, products_url):
    ...
    assert response.json() == {'detail':'Product no found with filter: 4fd7cd35-a3a0-4c1f-a78d-d24aa81e7dca'}

@pytest.mark.usefixtures("products_inserted")
async def test_controller_query_should_return_sucess(client, products_url, products_inserted):
